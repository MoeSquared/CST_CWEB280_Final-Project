...
from endpoints.assignment_endpoint import UPLOAD_FOLDER
    ...
    try:
        ...
        with Session() as session:
            ...
            if file and file.filename:
                ...
                file_path = os.path.join(UPLOAD_FOLDER, f"course_{course_validated.courseName}_{file.filename}")
                with open(file_path, "wb") as f:
                    content = await file.read()
                    f.write(content)

                new_course.filename = file.filename
                new_course.file_path = file_path
...
async def update_course(
    ...
    try:
        with Session() as session:
            ...
            if file and file.filename:
                os.makedirs(UPLOAD_FOLDER, exist_ok=True)
                file_path = os.path.join(UPLOAD_FOLDER, f"course_{course.courseName}_{file.filename}")
                with open(file_path, "wb") as f:
                    content = await file.read()
                    f.write(content)
