...
def validate_course_json(course):
    """Method to validate that a course JSON object is in a database-friendly format"""
    # Check for presence of necessary values
    required_keys = ["courseName", "credits", "startDate", "endDate", "daysOfWeek", "startTime", "endTime"]
    ...
    for key in ["startDate", "endDate"]:
        try:
            datetime.strptime(course[key], "%Y-%m-%d")
        except (ValueError, TypeError):
            raise ValueError(f"Course {key} is not a valid date")

    # Time: HH:MM:SS
    for key in ["startTime", "endTime"]:
        try:
            datetime.strptime(course[key], "%H:%M:%S")
        except (ValueError, TypeError):
            raise ValueError(f"Course {key} is not a valid time")

    # Test that course start date is before course end date
    course_start_date = datetime.strptime(course["startDate"], "%Y-%m-%d").date()
    course_end_date = datetime.strptime(course["endDate"], "%Y-%m-%d").date()
